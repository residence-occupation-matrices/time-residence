...
Brownian standard-deviation rate and enters the increment covariance as
``Delta t * sigma**2``.
"""
...
class MotionParameters:
    """Brownian rate and location-error standard deviation.

    With coordinates in metres and elapsed time in seconds, ``sigma`` has units
    metres per square-root second and ``location_error`` has units metres.
    """
...
def _contains_xy(geometry, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    except ImportError:  # Shapely 1.x compatibility
