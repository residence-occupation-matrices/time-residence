...
def normalize_ping_columns(
    ...
    """Return a copy with standard ping columns and valid coordinates.

    Tables whose latitude and longitude fields appear reversed are rejected
    unless ``repair_swapped_coordinates`` is enabled.
    """
    ...
    if swapped_signature:
        if not repair_swapped_coordinates:
            raise ValueError(
                "Coordinate header inversion detected: latitude and longitude appear "
                "to be reversed. Set repair_swapped_coordinates=True only after "
                "confirming the source schema."
            )
