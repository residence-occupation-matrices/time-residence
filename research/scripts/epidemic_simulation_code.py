...
# ruff: noqa: E741
"""
Created on Mon Sep 26 10:27:24 2022
...
)  # P1A and P2A use the same dates.
...
dat_one_alpha_FF = pd.read_csv(
    DATA_ROOT / "one_alpha_POBTOT" / "alphas_SF.csv"
)  # P1A and P2A use the same dates.
...
# common_agebs = np.intersect1d(working_agebs_FF, working_agebs_FS)
...
# var_dict1 = initialise_variables(7)
...
# %% plot the differences in infection counts for each ageb (Figure 8A)
...
plt.close(fig)
# plt.title('Difference in infections in FP_FP and in FP_SP')

# %%Find the proportion of agebs with negative difference counts

dif_FF_FS = np.subtract(commonI0_FF, commonI0_FS)
# np.all(dif_FF_FS[13,]<0.0,axis=0)

np.count_nonzero(dif_FF_FS[15,] < 0.0) / np.shape(dif_FF_FS)[1]

# %%plot the glibal infection differences (Figure 8B)
...
plt.close(fig)
# plt.title('Difference in global infections in FP_FP and in FP_SP')


# %%

####################Second period First Part and Second Period Second part##################
...
# N_bar = np.array([4,7,9,1,9,7,9]).reshape(7,1)
...
# %%plot the differences in infection counts for each ageb (Fgure 8C)
...
plt.close(fig)
# plt.title('Difference in infections in SP_FP and in SP_SP')
...
# %%%%%plot the glibal infection differences (Figure 8D)
...
####################Third period First Part and Third Period Second part##################
...
# %% plot the differences in infection counts for each ageb (Figure 8E)
I0_TF = sol_TF[:, 2, :, 0]
...
plt.close(fig)
# plt.title('Difference in infections in TP_FP and in TP_SP')
...
# %%plot the glibal infection differences (Figure 8F)
